...
def main():
    ...
    # create model
    print(
        "=> creating model based on '{}'".format('fashion detection network'))
    ...
    train_loader = data_loader(
        BatchSize=cfg.batch_size, NumWorkers=cfg.num_workers).train_loader
...
def train_model(train_loader, model, criterion, optimizer, epoch, epochs):
    ...
    for iter, traindata in enumerate(train_loader):
        ...
        train_inputs, train_labels, train_landmarks = torch.autograd.Variable(
            train_inputs.cuda(async=True), ).float(), torch.autograd.Variable(
                train_labels.cuda(
                    async=True)).float(), torch.autograd.Variable(
                        train_landmarks.cuda(async=True)).float()
