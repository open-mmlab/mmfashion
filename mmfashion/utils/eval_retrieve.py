...
def show_result(query_embeds, gallery_embeds, query_dict, gallery_dict):
    ...
    for qi, query_embed in enumerate(query_embeds):
        ...
        for gi, gallery_embed in enumerate(gallery_embeds):
            one_dist = cdist(
                query_embed.reshape(1, -1), gallery_embed.reshape(1, -1),
                'euclidean')
        ...
        if qi % 10 == 0:
            ...
            acc3, acc5, acc10, acc20 = 100 * float(top3) / total, 100 * float(
                top5) / total, 100 * float(top10) / total, 100 * float(
                    top20) / total
    ...
    print(top3, top5, top10, top20)
    acc3, acc5, acc10, acc20 = 100 * float(top3) / total, 100 * float(
        top5) / total, 100 * float(top10) / total, 100 * float(top20) / total
    print('top3 = %.4f, top5 = %.4f, top10 = %.4f, top20 = %.4f ' %
          (acc3, acc5, acc10, acc20))
...
query_dict = load_dict(
    '/newDisk/users/liuxin/mmfashion-prerelease/mmfashion/datasets/In-shop/Anno/query_idx2id.txt'
)
gallery_dict = load_dict(
    '/newDisk/users/liuxin/mmfashion-prerelease/mmfashion/datasets/In-shop/Anno/gallery_idx2id.txt'
)
