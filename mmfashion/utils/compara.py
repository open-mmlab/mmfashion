...
rf = open(
    '/newDisk/users/liuxin/mmfashion-prerelease/mmfashion/datasets/In-shop/Eval/list_eval_partition.txt'
).readlines()
img2id = {}
...
query_f = open(
    '/newDisk/users/liuxin/mmfashion-prerelease/mmfashion/datasets/In-shop/Anno/query_img.txt'
).readlines()
...
gallery_f = open(
    '/newDisk/users/liuxin/mmfashion-prerelease/mmfashion/datasets/In-shop/Anno/gallery_img.txt'
).readlines()
...
for i, embed_q in enumerate(query):
    ...
    if i % 100 == 0:
        print('top1= %d, top3= %d, top5= %d, top10 = %d' % (top1, top3, top5,
                                                            top10))

        print('top1= %.4f, top3= %.4f, top5= %.4f, top10= %.4f' %
              (float(top1) / total, float(top3) / total, float(top5) / total,
               float(top10) / total))
