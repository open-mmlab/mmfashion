...
class AttrNet(nn.Module):
    ...
    def forward(self, x, landmarks, single=False):
        ...
        first_branch_conv = first_branch_conv.view(
            first_branch_conv.size(0), -1)
        ...
        if single is not False:  # a single image(used for 'demo')
            ...
            both_branch = torch.cat((first_branch_out, roi_out.view(1, 4096)),
                                    0).view(-1)
            ...
            roi_out = ROIPooling(
                x, landmarks,
                self.second_branch_fc)  # n landmarks--n rois(512*6*6)
