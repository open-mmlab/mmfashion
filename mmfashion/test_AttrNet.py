...
def main():
    ...
    test_loader = data_loader(
        BatchSize=cfg.batch_size, NumWorkers=cfg.num_workers).test_loader
    if cfg.loss == 'MSE':
    ...
    for iter, test_data in enumerate(test_loader):
        ...
        test_inputs, test_labels, landmarks = torch.autograd.Variable(
            test_inputs.cuda(),
            volatile=True).float(), torch.autograd.Variable(
                test_labels.cuda(),
                volatile=True).float(), torch.autograd.Variable(
                    landmarks.cuda(), volatile=True)
        ...
        top3_TP_batch, top5_TN_batch, top5_TP_batch, top5_TN_batch = cal_accuracy(
    ...
    attr_count, image_count = count_attribute(
        cfg.TEST_LABEL_FILE)  # the ground truth

    for acc_i, acc in enumerate(top3_TP):
        top3_accuracy[acc_i] = 100 * (
            float(top3_TP[acc_i] + top3_TN[acc_i]) / float(image_count))
    for acc_i, acc in enumerate(top5_TP):
        top5_accuracy[acc_i] = 100 * (
            float(top5_TP[acc_i] + top5_TN[acc_i]) / float(image_count))
    ...
    print("top3 average accuracy : %.4f" %
          (float(sum(top3_accuracy)) / len(top3_accuracy)))
    ...
    for i, count in enumerate(top5_TP):
        print("%d th attribute %s : %d" % (i, attr[i], count))
    print("top5 average accuracy : %.4f" %
          (float(sum(top5_accuracy)) / len(top5_accuracy)))
