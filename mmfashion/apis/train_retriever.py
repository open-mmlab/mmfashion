...
def batch_processor(model, data, train_mode):
    ...
    losses = model(anchor, anchor_lm, pos,pos_lm, neg,neg_lm)
    loss, log_vars = parse_losses(losses)
    
    outputs = dict(
          loss=loss, log_vars=log_vars, num_samples=len(anchor.data))
    return outputs


def train_retriever(model, dataset, cfg, distributed=False, validate=False, logger=None):
    if logger is None:
