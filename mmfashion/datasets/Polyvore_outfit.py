...
class PolyvoreOutfitDataset(Dataset):
        ...
        for outfit in self.outfit_info:
            ...
            for j in range(set_len-1):
                for k in range(j+1, set_len):
                    pos_pairs.append([set_id, items[j]['item_id'], items[k]['item_id']])
        return pos_pairs

    @staticmethod
    def load_typespaces(typespace_fn):
        """ loads a mapping of pairs of types to the embedding used to compare them
            rand_typespaces: Boolean indicator of randomly assigning type specific spaces to their embedding
            num_rand_embed: number of embeddings to use when rand_typespaces is true
        """
    ...
    def parse_iminfo(question, im2index, id2im, gt=None):
        """ This is the same with "Learning Type-Aware Embeddings for Fashion Compatibility"
    ...
    def collect_compatibility_questions(self, compatibility_test_fn):
        ...
        for line in compatibility_rf:
            ...
            compat_question, _, _ = self.parse_iminfo(data[1:], self.item2index, self.setid2item)
    ...
    def test_fitb(self, embeds, metric):
        """ Returns the accuracy of the fill in the blank task
            ...
            metric: a function used to score the elementwise product
                    of a pair of embeddings, if None euclidean distance is used
        """
        correct = 0.
        n_questions = 0.
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            img1, desc1, has_text1, anchor_cate = self.load_train_item(anchor_im)
